...
SAVE_PATH = "/home/joseph/2-vertex-data/"
READ_PATH = "/home/joseph/new/"
...
def tile_logger(filePath, tileID, vertexList,
    ...
    VertexList[0]: Centre,  VertexList[n]: Vn
    '''
    latLonList =[]
    if coordsInfo != 0:
...
def square_tile_gen(array, centre, tileSz=64):
    ...
    sz = tileSz//2
    res = array[:, (x-sz):(x+sz), (y-sz):(y+sz)].copy()
    return res

def two_vertix_tiler(array, stride=1, tileSz=64,
                     savePath = "",filePrefix = "",
                     coordsInfo = 0, tifName=""):
    '''
    ...
    for i in range(sz, H-sz, tileSz*stride):
        for j in range(sz, W-sz, tileSz*stride):
            ...
            np.save(savePath+filePrefix+"-"+str(i)+"-"+str(j)+ "-V1.npy",V1.astype(np.uint8)    , allow_pickle=False)
            np.save(savePath+filePrefix+"-"+str(i)+"-"+str(j)+ "-V2.npy",V2.astype(np.uint8)    , allow_pickle=False)
            np.save(savePath+filePrefix+"-"+str(i)+"-"+str(j)+"-Cen.npy",Cen.astype(np.uint8), allow_pickle=False)
...
for root, dirc, files in os.walk(READ_PATH):
    savePath = SAVE_PATH+"/"+root.replace(READ_PATH,"")+"/"
